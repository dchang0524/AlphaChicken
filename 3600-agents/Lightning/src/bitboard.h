...
inline bool is_unclaimed_egg(const GameState& state, Position pos, bool for_player) {
        Bitboard bit = state.pos_to_bitboard(pos);

        if (for_player) {
            // Egg of MY color = (parity matches my parity)
            bool correct_parity = (BitboardOps::is_even_square(pos) == (state.player_even_chicken == 0));
            bool is_unclaimed   = !(bit & state.eggs_player) && !(bit & state.eggs_enemy);
            return correct_parity && is_unclaimed;
        } else {
            // Egg of OPP color = (parity matches enemy parity)
            bool correct_parity = (BitboardOps::is_even_square(pos) == (state.enemy_even_chicken == 0));
            bool is_unclaimed   = !(bit & state.eggs_player) && !(bit & state.eggs_enemy);
            return correct_parity && is_unclaimed;
        }
    }
