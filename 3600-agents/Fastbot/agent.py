...
# UPDATE IMPORT
from .heuristics import move_order, move_order_fast 
...
class PlayerAgent:
    def __init__(self, initial_board: board_mod.Board, time_left: Callable):
        ...
        self.killer_moves = [None] * 20

        self.bfs_q_x = np.zeros(dim * dim, dtype=np.int32)
        self.bfs_q_y = np.zeros(dim * dim, dtype=np.int32)
        self.bfs_dist_me = np.full((dim, dim), -1, dtype=np.int32)
        self.bfs_dist_opp = np.full((dim, dim), -1, dtype=np.int32)
    ...
    def _warmup(self):
        ...
        d_me = numba_bfs(0, 0, self.obs_me, self.dim, self.bfs_q_x, self.bfs_q_y, self.bfs_dist_me)
        d_opp = numba_bfs(self.dim-1, self.dim-1, self.obs_opp, self.dim, self.bfs_q_x, self.bfs_q_y, self.bfs_dist_opp)
    ...
    def _get_voronoi(self, cur_board: board_mod.Board) -> VoronoiInfo:
        ...
        dist_me = numba_bfs(mx, my, self.obs_me, dim, self.bfs_q_x, self.bfs_q_y, self.bfs_dist_me)
        dist_opp = numba_bfs(ox, oy, self.obs_opp, dim, self.bfs_q_x, self.bfs_q_y, self.bfs_dist_opp)
        ...
        (my_owned, opp_owned, contested, max_cd, min_cd, min_ed, 
         my_v, opp_v, frag_score, cu, cr, cd, cl) = stats
    ...
    def negamax(self, cur_board, depth, alpha, beta, time_left, even_trap, odd_trap, cum_risk):
        ...
        # --- OPTIMIZATION: LAZY MOVE ORDERING ---
        # We DO NOT call _get_voronoi() here. 
        # We use move_order_fast which sorts by simple heuristics.
        ...
        ordered_moves = move_order_fast(cur_board, moves, killer_move=k_move)
    ...
    def _search_root(self, board, moves, depth, time_left):
        # Root node still uses FULL Voronoi move ordering for the best start
        alpha, beta = -INF, INF
        best_move = None
        best_val = -INF
    ...
    # ... (Keep helper methods _update_trap_knowledge_state, etc. same as before) ...
