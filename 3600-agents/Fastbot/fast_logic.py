...
def numba_voronoi(dist_me, dist_opp, my_x, my_y, my_even, opp_even, dim):
    ...
    # Directional counts for fragmentation
    cu, cr, cd, cl = 0, 0, 0, 0
    q1, q2, q3, q4 = 0, 0, 0, 0
    ...
    if total_dir > 1:
        ...
        if cardinal_frag > 1.0: cardinal_frag = 1.0
    ...
    if q_total > 0:
        ...
        if q3 > mq: mq = q3
        if q4 > mq: mq = q4
        quad_spread = 1.0 - (mq / q_total)
        q_dirs = 0
        if q1 > 0: q_dirs += 1
        if q2 > 0: q_dirs += 1
        if q3 > 0: q_dirs += 1
        if q4 > 0: q_dirs += 1
        quad_score = 0.5 * quad_spread + 0.5 * ((q_dirs - 1) / 3.0)

    frag_score = 0.5 * cardinal_frag + 0.5 * quad_score

    # FIXED: Use the correct variable names here
    if min_contested_dist == 999: min_contested_dist = dim * dim
    if min_egg_dist == 999: min_egg_dist = dim * dim

    return (
        my_owned, opp_owned, contested, 
        max_contested_dist, min_contested_dist, min_egg_dist,
        my_voronoi, opp_voronoi,
        frag_score, # Returned directly
        cu, cr, cd, cl 
    )
