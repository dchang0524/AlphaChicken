...
engine_path = os.path.abspath(os.path.join(current_dir, "../../engine"))
...
class PlayerAgent:
    def __init__(self, initial_board: board_mod.Board, time_left: Callable):
        with open("bobby_init.txt", "a") as f: f.write("Init start\n")
        ...
        with open("bobby_init.txt", "a") as f: f.write("Init done\n")
    ...
    def play(
        ...
        # print(self.known_traps)
        # print(sensor_data)
        # self.trap_belief.debug_print()
    ...
    def _search_root(
        ...
        for mv in ordered_moves:
            ...
            for (even_trap, odd_trap, weight) in scenarios:
                ...
                # Root POV:
                #   - cumulative risk to root side so far = 0 + delta_risk
                #   - child POV must see the NEGATED risk.
    ...
    def negamax(
        ...
        # --- terminal & time cutoff (unchanged) ---
        ...
        if depth == 0 or not moves:
            ...
            v_leaf = g_leaf + cum_risk + self.TURD_WEIGHT * cur_board.chicken_player.get_turds_left()
            self.tt[key] = TTEntry(
                value=g_leaf,
                depth=depth,
                flag="EXACT",
        ...
        for mv in ordered_moves:
            ...
            child, delta_risk = self._simulate_move_with_traps(
                cur_board, mv, even_trap, odd_trap
            )

            # risk flip when POV flips
