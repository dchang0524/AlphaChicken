...
class PlayerAgent:
    ...
    def evaluate(self, cur_board: board.Board, my_dist: dict[tuple[int, int], int], opp_dist: dict[tuple[int, int], int]) -> float:
        """
        Evaluates the board state.
        Fix: "Voronoi" now means "Total Parity Yield" (Eggs Laid + Empty Squares Controlled).
        We do NOT skip occupied squares (except turds).
        """
        dim = cur_board.game_map.MAP_SIZE
        
        # Determine Parity Identity
        # "Is the Hero (the bot) currently the one moving?"
        current_turn_is_even = cur_board.can_lay_egg_at_loc((0, 0))
        is_hero_moving = (current_turn_is_even == self.isEvenChicken)
        
        # --- Root State Reference (For calculating deltas/improvement) ---
        # We need to know what the board looked like at the start of the search
        # to see if we improved our position.
        r_opp_eggs = self.state.eggs_enemy if is_hero_moving else self.state.eggs_player
        r_my_eggs = self.state.eggs_player if is_hero_moving else self.state.eggs_enemy
        
        # Distances: 
        # _r suffixes indicate Root (Reference) state
        my_dist_r  = self.dist_player if is_hero_moving else self.dist_enemy
        opp_dist_r = self.dist_enemy if is_hero_moving else self.dist_player
        ...
        # --- Metrics to Compute ---
        new_me = 0            # My Material Score (Weighted)
        new_opp = 0           # Opp Material Score (Weighted)
        
        new_opp_voronoi = 0   # Current Board: Opponent Total Control (Eggs + Space)
        opp_voronoi = 0       # Root Board: Opponent Total Control (Reference)
        
        new_my_voronoi = 0    # Current Board: My Total Control (Eggs + Space)
        my_voronoi = 0        # Root Board: My Total Control (Reference)

        new_blocks = 0        # Squares we blocked them from reaching
        impossible_contest = 0 # Squares we can't reach to contest
        contested = 0
        ...
        for x in range(dim):
            for y in range(dim):
                ...
                # 1. TURDS ARE DEAD SPACE (Skip)
                # If a square has a turd (current or root), it's worth nothing to anyone.
                if pos in cur_board.turds_player or pos in cur_board.turds_enemy:
                    continue
                
                # --- MATERIAL SCORE (Corner Bonuses) ---
                if pos in cur_board.eggs_player:
                    new_me += 1
                    if pos in {(0,0), (0,dim-1), (dim-1,0), (dim-1,dim-1)}:
                        new_me += 2
                if pos in cur_board.eggs_enemy:
                    new_opp += 1
                    if pos in {(0,0), (0,dim-1), (dim-1,0), (dim-1,dim-1)}:
                        new_opp += 2

                # --- PARITY CHECKS ---
                is_opp_parity = (((x + y) % 2 == 0) == opp_even)
                is_my_parity = (((x + y) % 2 == 0) == my_even)
                
                # Fetch BFS distances
                d_me = my_dist.get(pos)
                d_opp = opp_dist.get(pos)
                d_me_r = my_dist_r.get(pos)
                d_opp_r = opp_dist_r.get(pos)

                # --- OPPONENT VORONOI CALCULATION ---
                if is_opp_parity:
                    # A. Current Board State (New)
                    if pos in cur_board.eggs_enemy:
                        # They laid an egg here. They control it 100%.
                        new_opp_voronoi += 1
                    elif pos in cur_board.eggs_player:
                        # I laid an egg here (somehow, shouldn't happen on opp parity usually).
                        pass
                    else:
                        # Empty Square: Do they reach it faster?
                        if d_opp is not None:
                            if d_me is None or d_opp <= d_me:
                                new_opp_voronoi += 1
                                
                            # Heuristic: If they can't reach it now, but could before, we blocked them.
                            if d_opp is None and d_opp_r is not None:
                                new_blocks += 1
                        
                        if d_me is None and d_me_r is not None and d_opp is not None:
                             impossible_contest += 1

                    # B. Root Board State (Old/Reference)
                    if pos in r_opp_eggs:
                        opp_voronoi += 1
                    elif pos in r_my_eggs:
                        pass
                    else:
                        # Empty in Root: Did they control it then?
                        if d_opp_r is not None:
                            if d_me_r is None or d_opp_r <= d_me_r:
                                opp_voronoi += 1

                # --- MY VORONOI CALCULATION ---
                if is_my_parity:
                    # A. Current Board State (New)
                    if pos in cur_board.eggs_player:
                        new_my_voronoi += 1
                    elif pos in cur_board.eggs_enemy:
                        pass
                    else:
                        # Empty: Do I reach it faster?
                        if d_me is not None:
                    ...
                    if pos in r_my_eggs:
                         my_voronoi += 1
                    elif pos in r_opp_eggs:
                         pass
                    else:
                         if d_me_r is not None:
                            if d_opp_r is None or d_me_r < d_opp_r:
                                my_voronoi += 1
        ...
        base_diff = new_me - new_opp
        
        # "Potential Contest": The space they control that ISN'T unreachable for us.
        potential_contest = new_opp_voronoi - impossible_contest 
        
        # Phase 3: Finish (Greedy / Absolute)
        if moves_left <= ENDGAME_TURNS or potential_contest <= OPP_TIGHT:
            return (
                100.0 * base_diff         # Eggs are everything
                - 2.0 * new_opp_voronoi   # Tie-breaker: Keep them squeezed
                # NO DELTA HERE. We don't care about "improvement," only "winning."
            )

        # Phase 1: Positional Squeeze (Tal Mode / High Delta)
        if moves_left >= MIDGAME_TURNS and potential_contest >= OPP_LOOSE:
            # Calculate Delta: (Old - New). Positive means we shrank them / grew us.
            voronoi_delta = (opp_voronoi - new_opp_voronoi) + (new_my_voronoi - my_voronoi)
            
            return (
                -2.0 * new_opp_voronoi    # Absolute: Reduced from -5.0. It's a baseline.
                + 5.0 * voronoi_delta     # Delta: Massive boost. REWARDS THE ACT OF CUTTING.
                + 2.0 * new_blocks        # Tactical: High reward for creating dead ends
                + 2.0 * base_diff         # Material: Keep it relevant so we don't sacrifice too many eggs
                - 2.0 * len(cur_board.turds_player)
            )

        # Phase 2: Conversion (Balanced)
        # Transition phase: We start caring more about the absolute state again.
        return (
            -4.0 * new_opp_voronoi       # Absolute takes over again
            + 2.0 * base_diff
            + 1.0 * new_blocks
        )
