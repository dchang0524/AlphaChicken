...
class PlayerAgent:
    def __init__(self, initial_board: board_mod.Board, time_left: Callable):
        ...
        # Scale for path-risk penalty. 
        # Will be updated dynamically in play()
    ...
    def play(
        ...
        # Raised threshold slightly to 0.40 to reduce fear of low-probability ghosts
        self._update_potential_traps(board, threshold=0.30)
        
        # --------------------------------------------------------
        # BOBBY'S DYNAMIC WEIGHT LOGIC
        # --------------------------------------------------------
        ...
        # Cap the budget
        if start_time > 10.0: 
             time_budget = min(time_budget, 3.5) 
    ...
    def _update_potential_traps(self, board: board_mod.Board, threshold: float = 0.30) -> None:
    ...
    def _undo_move_inplace(self, board: board_mod.Board, move: Tuple, undo_data: Tuple) -> None:
        ...
        if triggered_trap:
             # Manually subtract because we used increment_eggs_laid which might have side effects? 
             # Actually board.py uses += so -= is safe.
    ...
    def _search_root(
        ...
        for mv in ordered_moves:
            ...
            for (even_trap, odd_trap, weight) in scenarios:
                ...
                delta_risk = 0.0
                # Apply penalty if it matches potential trap list
                ...
                if (is_even_sq and (px, py) not in self.potential_even) or \
                   (not is_even_sq and (px, py) not in self.potential_odd):
                    # Check if this square is statistically risky
                    if prob_here > 0:
    ...
    def negamax(
        ...
        if not moves:
             # Loss (game over not triggered yet?)
             return -INF
        ...
        for mv in ordered_moves:
            ...
            # Simplified risk check for internal nodes (consistent with root)
            is_even_sq = ((px + py) % 2 == 0)
            if (is_even_sq and (px, py) not in self.potential_even) or \
               (not is_even_sq and (px, py) not in self.potential_odd):
                 if prob_here > 0:
