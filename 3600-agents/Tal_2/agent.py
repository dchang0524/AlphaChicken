    ...
    def evaluate(self, cur_board: board.Board) -> float:
        ...
        for x in range(dim):
            for y in range(dim):
                ...
                if (d_me is None and d_opp is not None) or (d_opp is not None and d_opp <= d_me + 2):
                    opp_voronoi += 1
        ...
        OPP_TIGHT = 8     # they are very constricted
        OPP_LOOSE = 16     # they have a big region
        ...
        potential_contest = opp_voronoi - impossible_contest #eggs we can acatually fight for
        if moves_left <= ENDGAME_TURNS or potential_contest <= OPP_TIGHT:
            ...
            #self.simple_log(cur_board.turn_count, potential_contest, "ENDGAME" )
            return (
        ...
        if moves_left >= MIDGAME_TURNS and potential_contest >= OPP_LOOSE:
            #self.simple_log(cur_board.turn_count, potential_contest, "CRUSH" )

            return (
                -5.0 * opp_voronoi    # primary: shrink their egg Voronoi
                + 1.5 * blocks
        ...
        #self.simple_log(cur_board.turn_count, potential_contest, "MIX")
        return (
