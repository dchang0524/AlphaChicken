import math
from typing import List, Tuple, Dict, Deque, Optional, Any
from collections import deque

from game import board as board_mod  # type: ignore
from .voronoi import analyze, VoronoiInfo, OWNER_NONE, OWNER_ME, OWNER_OPP
...
# Use a high but safe infinity so we don't overflow when summing
INF = HeuristicWeights.W_LOSS_PENALTY 
...
def evaluate(cur_board: board_mod.Board, vor: VoronoiInfo, trap_belief : TrapdoorBelief) -> float:
    ...
    STRATEGY:
    1. Midgame: Aggressively claim space (Voronoi) using weights.
    2. Endgame: 'Physical Limit' cap forces the bot to realize that 
       excess space is worthless if it doesn't have turns to lay eggs.
       This triggers the "Cash Out" behavior.
    """
    ...
    # --- THE PHYSICAL LIMIT (CASH OUT LOGIC) ---
    # This represents the hard limit of how many eggs we can physically lay
    # with the turns remaining. 
    # Logic: If I own 20 squares but only have 2 turns, those 20 squares 
    # are worth at most 1 egg (since it takes 2 turns to lay one).
    # We add 0.5 buffer for parity/movement flexibility.
    physical_limit = moves_left / 2.0 + 0.5
    ...
    # STRICT CAP: Forces conversion.
    # If potential > limit, the only way to increase score is to move
    # points from "Potential" to "Base Eggs" (by laying them).
    my_potential = min(my_safe_term + my_vor_term, physical_limit)
    
    # 4. Egg Dist (Penalty) - Only apply if we need to find an egg spot
    ...
    # Aggressive Penalty: Forces bot to stay near the "Frontier".
    # The penalty increases as phase_mat decreases (Late game = fight harder).
    my_contested = -HeuristicWeights.W_CONTESTED_SIG * vor.sum_weighted_contested_dist * \
                   ((1 - HeuristicWeights.CONTESTED_OPENNESS_CORRELATION) + \
                    (1 - phase_mat) * HeuristicWeights.CONTESTED_OPENNESS_CORRELATION)
    
    # 12. Loss Prevention (Penalty)
    my_loss = 0.0
    # Theoretical max: Current Eggs + Max Future Eggs.
    # If my theoretical max < opponent's current score, I have already lost.
    my_theo_max = my_eggs + physical_limit
    if my_theo_max < opp_eggs:
        my_loss = -HeuristicWeights.W_LOSS_PENALTY
        
    my_total = (
        my_eggs + # Base (Real points always matter 1.0)
        my_potential + 
        my_egg_dist_score + 
        my_turd_save + 
        my_bad_turd + 
    ...
    opp_potential = min(opp_safe_term + opp_vor_term, physical_limit)
    ...
    # 9. Contested Dist
    opp_contested = -HeuristicWeights.W_CONTESTED_SIG * vor.opp_sum_weighted_contested_dist * \
                    ((1 - HeuristicWeights.CONTESTED_OPENNESS_CORRELATION) + \
                     (1 - phase_mat) * HeuristicWeights.CONTESTED_OPENNESS_CORRELATION)
    
    # 12. Loss Prevention
    opp_loss = 0.0
    opp_theo_max = opp_eggs + physical_limit
...
def get_weight_gradient(cur_board: board_mod.Board, vor: VoronoiInfo, trap_belief : TrapdoorBelief) -> Dict[str, float]:
    """
    Gradient of Magnus evaluation. Used for tuning/RL.
    """
    if cur_board.is_game_over():
        return {}
    ...
    # W_SAFE_EGG gradient
    # If we are capped, the gradient is capped
    grads["W_SAFE_EGG"] = min(vor.my_safe_potential_eggs, physical_limit)
    
    # W_VORONOI_EGG gradient
    grads["W_VORONOI_EGG"] = min(vor.my_reachable_potential_eggs, physical_limit)
    ...
    my_eggs  = cur_board.chicken_player.eggs_laid
    opp_eggs = cur_board.chicken_enemy.eggs_laid
    theoretical_max = my_eggs + physical_limit
...
def debug_evaluate(
    ...
    Recomputes local owner grid and my region sizes for visualization.
    """
    dim = cur_board.game_map.MAP_SIZE
    ...
    # ------------------------------------------------------------
    # SCORING LOGIC (UPDATED WITH PHYSICAL LIMIT)
    # ------------------------------------------------------------
    ...
    # opp_moves_left approximation if needed, usually syncs
    
    total_moves = cur_board.MAX_TURNS
    
    # PHYSICAL LIMIT for Debug
    physical_limit = my_moves_left / 2.0 + 0.5
    ...
    my_contested = -HeuristicWeights.W_CONTESTED_SIG * vor.sum_weighted_contested_dist * \
                   ((1 - HeuristicWeights.CONTESTED_OPENNESS_CORRELATION) + \
                    (1 - my_phase) * HeuristicWeights.CONTESTED_OPENNESS_CORRELATION)
    ...
    opp_contested = -HeuristicWeights.W_CONTESTED_SIG * vor.opp_sum_weighted_contested_dist * \
                    ((1 - HeuristicWeights.CONTESTED_OPENNESS_CORRELATION) + \
                     (1 - opp_phase) * HeuristicWeights.CONTESTED_OPENNESS_CORRELATION)
...
def move_order(
    ...
    # Basic filtering: If there are EGG moves, filter out PLAIN moves
    # This acts as a pruning step for 'lazy' moves in rich environments
