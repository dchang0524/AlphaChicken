...
class VoronoiInfo:
    """
    Lightweight Board-dependent Voronoi / frontier analysis.
    Stores ONLY scalar metrics to prevent memory accumulation in search trees.
    """
    ...
    def __init__(
        ...
        opp_owned: int,
        contested: int,
        average_contested_dist: float,
        max_contested_dist: int,
        min_contested_dist: int,
        min_egg_dist: int,
        my_voronoi: int,
        opp_voronoi: int,
        contested_up: int,
        ...
        contested_down: int,
        contested_left: int,
        my_reachable_potential_eggs: int,
        my_safe_potential_eggs: int,
        sum_weighted_contested_dist: float,
        bad_turd_count: int,
        opp_reachable_potential_eggs: int,
        opp_safe_potential_eggs: int,
        opp_sum_weighted_contested_dist: float,
        opp_bad_turd_count: int,
        opp_min_egg_dist: int,
    ):
...
def analyze(
    ...
    # 2D owner grid - NEEDED FOR CALCULATION, but NOT stored in class
    owner: List[List[int]] = [
    ...
    for x in range(dim):
        for y in range(dim):
            ...
            # Note: We only increment owned counts if strictly exclusive reachability
            # or based on specific rules, but sticking to your original logic:
            if reachable_me and not reachable_opp:
                my_owned += 1
            elif reachable_opp and not reachable_me:
                # Optional: track opp_owned for exclusive territory if needed
                pass
    ...
    # compute region-level stats locally
    ...
    # --------------------------
    # Only checks moves where dist >= 0 to ensure we aren't walking into walls/off map
